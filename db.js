...
// ==================== ESTADÍSTICAS ====================
// Las métricas se guardan de forma persistente. En Render se recomienda
// Upstash Redis; en desarrollo/local se usa data/stats.json.
const EMPTY_STATS = {
    totalRequests: 0,
...
async function incrementStats({ endpoint, statusCode, responseTimeMs }) {
    ...
    let stats;
    try {
        stats = normalizeStats(JSON.parse(fs.readFileSync(filePath, 'utf-8')));
    } catch {
        stats = { ...EMPTY_STATS };
    }
...
async function getStats({ days = 7 } = {}) {
    ...
    if (USE_REDIS) {
        const [globalRaw, endpointsRaw] = await Promise.all([
        ...
        for (let i = 0; i < (globalRaw || []).length; i += 2) {
            global[globalRaw[i]] = Number(globalRaw[i + 1]) || 0;
        }

        const endpointRequests = {};
        for (let i = 0; i < (endpointsRaw || []).length; i += 2) {
            endpointRequests[endpointsRaw[i]] = Number(endpointsRaw[i + 1]) || 0;
        }
        ...
        return normalizeStats({ ...global, endpointRequests, daily });
    }

    let stats;
    try {
        stats = normalizeStats(JSON.parse(fs.readFileSync(localPath('stats'), 'utf-8')));
    } catch {
        stats = { ...EMPTY_STATS };
    }
