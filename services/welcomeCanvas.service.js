...
<defs>
<linearGradient id="bg" x1="0" y1="0" x2="1" y2="1"><stop stop-color="${theme.bg1}"/><stop offset="1" stop-color="${theme.bg2}"/></linearGradient>
<radialGradient id="g1"><stop stop-color="${theme.glow}" stop-opacity=".35"/><stop offset="1" stop-color="${theme.glow}" stop-opacity="0"/></radialGradient>
<radialGradient id="g2"><stop stop-color="${theme.glow2}" stop-opacity=".28"/><stop offset="1" stop-color="${theme.glow2}" stop-opacity="0"/></radialGradient>
<linearGradient id="line"><stop stop-color="${theme.glow}" stop-opacity="0"/><stop offset=".5" stop-color="${theme.glow}" stop-opacity=".9"/><stop offset="1" stop-color="${theme.glow}" stop-opacity="0"/></linearGradient>
</defs>
<rect width="100%" height="100%" fill="url(#bg)"/>
<circle cx="180" cy="120" r="300" fill="url(#g1)"/><circle cx="1050" cy="500" r="360" fill="url(#g2)"/>
${stars}
<rect x="70" y="70" width="1060" height="490" rx="42" fill="#000" opacity=".18" stroke="${theme.glow}" stroke-opacity=".35" stroke-width="2"/>
<rect x="110" y="182" width="980" height="2" fill="url(#line)"/><rect x="110" y="515" width="980" height="2" fill="url(#line)"/>
</svg>`);
}
...
function avatarFrameSvg(theme) {
    return Buffer.from(`<?xml version="1.0" encoding="UTF-8"?>
<svg width="300" height="300" viewBox="0 0 300 300" xmlns="http://www.w3.org/2000/svg">
<defs><radialGradient id="a"><stop stop-color="${theme.glow}" stop-opacity=".9"/><stop offset="1" stop-color="${theme.glow2}" stop-opacity=".2"/></radialGradient></defs>
<circle cx="150" cy="150" r="144" fill="none" stroke="url(#a)" stroke-width="9"/>
<circle cx="150" cy="150" r="132" fill="none" stroke="${theme.glow}" stroke-opacity=".55" stroke-width="3"/>
</svg>`);
}

function circleMask(size = 300) {
...
async function generateWelcomeCanvas(options = {}) {
    const styleValue = String(options.style || 'divine').toLowerCase();
    const style = STYLES.has(styleValue) ? styleValue : 'divine';
    ...
    if (avatarBuffer) {
        const avatar = await sharp(avatarBuffer)
            .resize(270, 270, { fit: 'cover', position: 'centre' })
            .composite([{ input: circleMask(270), blend: 'dest-in' }])
            .png()
            .toBuffer();
        composites.push({ input: avatar, left: 165, top: 205 });
        composites.push({ input: avatarFrameSvg(theme), left: 150, top: 190 });
    } else {
        composites.push({ input: avatarFrameSvg(theme), left: 150, top: 190 });
    }
