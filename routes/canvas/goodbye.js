...
const text = (v, fallback, max) => { const s = String(v ?? '').trim().replace(/\s+/g, ' '); return (s || fallback).slice(0, max); };
...
router.post('/', upload.single('avatar'), async (req, res) => {
  try {
    ...
    const image = await generateWelcomeCanvas({ style, avatarBuffer:req.file?.buffer, avatarUrl,
      username:text(param(req,'username'),'Miembro',34), groupName:text(param(req,'groupName'),'Nuestro grupo',40), members:text(param(req,'members'),'0 miembros',12),
      message:text(param(req,'message'),'Hasta pronto',55), date:text(param(req,'date'),new Date().toLocaleDateString('es-PE'),24), title:text(param(req,'title'),'GOODBYE',32), footer:text(param(req,'footer'),'✦ FamilyBot-MD ✦',30) });
...
router.meta = { title:'Goodbye Canvas', description:'Genera una tarjeta de despedida para un miembro de un grupo de WhatsApp.', icon:'fas fa-door-open', method:'POST', fields:[
 {name:'avatar',label:'Avatar',type:'file'},{name:'avatarUrl',label:'Avatar URL',type:'url',placeholder:'https://...'},{name:'username',label:'Usuario',type:'text',placeholder:'Nombre'},{name:'groupName',label:'Grupo',type:'text',placeholder:'Nombre del grupo'},{name:'members',label:'Miembros',type:'text',placeholder:'25'},{name:'message',label:'Mensaje',type:'text',placeholder:'Hasta pronto'},{name:'date',label:'Fecha',type:'text',placeholder:'26/08/2026'},{name:'title',label:'Título',type:'text',placeholder:'GOODBYE'},{name:'footer',label:'Pie',type:'text',placeholder:'✦ FamilyBot-MD ✦'},{name:'style',label:'Estilo',type:'select',options:[...STYLES].map(v=>({value:v,label:v})),default:'dark'} ], resultType:'image', resultField:'result.url', directImage:true };
