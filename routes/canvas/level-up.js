...
router.post('/', upload.single('avatar'), async (req,res) => {
  try {
    ...
    const image = await generateWelcomeCanvas({ style, avatarBuffer:req.file?.buffer, avatarUrl,
      username:t(p(req,'username'),'Usuario',34), groupName:t(p(req,'groupName'),'¡Felicidades!',40), members:level, membersLabel:'NIVEL',
  ...
  } catch(e) { console.error('[LEVEL UP CANVAS ERROR]',e); const status=/avatar|imagen|image|HTTP|URL|límite|MB|dirección local|privada|tardó demasiado|vacío/i.test(e.message||'')?422:500; return res.status(status).json({status:false,creator:'FamilyBot-MD',message:status===422?e.message:'No se pudo generar el Level Up Canvas.'}); }
});
