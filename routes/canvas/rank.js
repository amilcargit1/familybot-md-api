...
router.post('/',upload.single('avatar'),async(req,res)=>{try{
  ...
  const image=await generateWelcomeCanvas({style,avatarBuffer:req.file?.buffer,avatarUrl,username:t(p(req,'username'),'Usuario',34),groupName:t(p(req,'groupName'),'Ranking',40),members:rank,membersLabel:'RANK',message:score,date:period,title:t(p(req,'title'),'RANKING',32),footer:t(p(req,'footer'),'✦ FamilyBot-MD ✦',30)});
...
}catch(e){console.error('[RANK CANVAS ERROR]',e);const status=/avatar|imagen|image|HTTP|URL|límite|MB|dirección local|privada|tardó demasiado|vacío/i.test(e.message||'')?422:500;return res.status(status).json({status:false,creator:'FamilyBot-MD',message:status===422?e.message:'No se pudo generar el Rank Canvas.'});}});
