...
router.post('/register', async (req, res) => {
    ...
    if (!username || !email || !password) return res.status(400).json({ status: false, message: 'Faltan datos: username, email, password' });
    const exists = (await db.findUser('email', email)) || (await db.findUser('username', username));
    if (exists) return res.status(400).json({ status: false, message: 'Ese usuario o correo ya existe' });
...
router.post('/login', async (req, res) => {
    ...
    if (!email || !password) return res.status(400).json({ status: false, message: 'Faltan datos: email, password' });
    if (safeCompare(email, ADMIN.email) && safeCompare(password, ADMIN.password)) {
        return res.json({ status: true, data: { username: ADMIN.username, email: ADMIN.email, key: ADMIN.key, plan: 'ADMIN VIP', role: 'admin' } });
    }
...
router.get('/stats', async (req, res) => {
    try {
        ...
        const daily = Object.entries(stats.daily).map(([date, value]) => ({
            date,
            requests: value.totalRequests,
            successful: value.successfulRequests,
            failed: value.failedRequests,
            averageResponseTimeMs: value.totalRequests ? Math.round(value.totalResponseTimeMs / value.totalRequests) : 0
        }));
        ...
        res.json({
            ...
            users,
            endpoints: Array.isArray(req.app.locals.apiEndpoints) ? req.app.locals.apiEndpoints.length : 0,
            requests: {
