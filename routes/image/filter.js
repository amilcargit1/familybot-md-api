...
const upload = multer({
    ...
    fileFilter: (req, file, cb) => {
        // Some WhatsApp wrappers use application/octet-stream. Sharp still
        // validates the actual image, so accepting it here keeps the endpoint
        // compatible with more bot frameworks.
...
async function applyFilter(input, filter, intensity) {
    ...
    switch (filter) {
        ...
        case 'duotone': {
            ...
            `);
            return image.grayscale().composite([{ input: overlay, blend: 'color' }]);
        }
        case 'warm':
            return image.modulate({ saturation: 1.12, brightness: 1.04 }).tint('#ffd0a3');
        case 'cool':
            return image.modulate({ saturation: 1.05, brightness: 1.02 }).tint('#9fc8ff');
        case 'vignette': {
            const { width, height } = await imageSize(image);
        ...
        case 'rainbow':
            return image.modulate({ saturation: number(intensity, 1.8, 1, 3), hue: 25 });
        case 'glitch': {
            const { width, height } = await imageSize(image);
            ...
            `);
            return image.modulate({ saturation: 1.35, brightness: 1.02 }).composite([{ input: overlay, blend: 'screen' }]);
        }
        case 'frost':
            return image.modulate({ saturation: 0.8, brightness: 1.12 }).tint('#d8efff').linear(1.04, 4);
        case 'night':
            return image.modulate({ saturation: 0.72, brightness: 0.72 }).tint('#243b72').linear(1.05, -4);
        case 'sunset':
            return image.modulate({ saturation: 1.35, brightness: 1.03 }).tint('#ff9b5e').linear(1.04, 3);
        case 'film':
            return image.modulate({ saturation: 0.88, brightness: 1.02 }).linear(1.12, -12).tint('#e8c69a');
        default:
    ...
    try {
        ...
        if (output.length > MAX_OUTPUT_SIZE) {
            return res.status(413).json({
                status: false,
                creator: 'FamilyBot-MD',
                message: 'La imagen procesada es demasiado grande. Usa una imagen de menor resolución.'
            });
        }
        ...
        if (String(req.query.format).toLowerCase() === 'image') {
            return res.status(200)
                ...
                .set('Content-Length', String(output.length))
        ...
        const isImageError = /Input|image|format|pixel|corrupt|unsupported|Vips/i.test(message);
...
router.use((error, req, res, next) => {
    ...
    if (error) {
        return res.status(400).json({
            status: false,
            creator: 'FamilyBot-MD',
            message: error.message || 'Archivo no válido.'
        });
    }
...
router.meta = {
    ...
    description: 'Aplica filtros y efectos a imágenes recibidas por multipart/form-data. Compatible con bots de WhatsApp.',
    icon: 'fas fa-image',
    method: 'POST',
    fields: [
        ...
        { name: 'intensity', label: 'Intensidad', type: 'text', placeholder: 'Opcional' }
    ],
    resultType: 'image',
    resultField: 'result.url',
    example: 'POST /api/image/filter?apiKey=TU_API_KEY&format=image'
};
